...
class MessengerServiceProvider extends ServiceProvider
{
    ...
    protected function configure()
    {
        $this->mergeConfigFrom(
            base_path('vendor/muyaedward/messenger/config/config.php'),
            'messenger'
        );
    }
    ...
    protected function offerPublishing()
    {
        if ($this->app->runningInConsole()) {
            $this->publishes([
                base_path('vendor/muyaedward/messenger/config/config.php') => config_path('messenger.php'),
            ], 'config');

            $this->publishes([
                base_path('vendor/muyaedward/messenger/migrations') => base_path('database/migrations'),
            ], 'migrations');
        }
    }
